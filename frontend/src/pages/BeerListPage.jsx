...
function BeerListPage() {
  ...
  const handleDelete = async (noteId) => {
    if (
      window.confirm(
        "Bist du sicher, dass du diese Bewertung löschen möchtest?"
      )
    ) {
      try {
  ...
  if (!user) {
    return (
      <div style={{ textAlign: "center" }}>
        ...
        <p>
          Bitte <Link to="/login">logge dich ein</Link> oder{" "}
          <Link to="/register">registriere dich</Link>, um deine
          Bier-Bewertungen zu sehen und zu verwalten.
        </p>
      </div>
    );
  }
    ...
    <div>
      ...
      {notes.length === 0 ? (
        <p style={{ textAlign: "center" }}>
          Du hast noch keine Biere bewertet.{" "}
          <Link to="/add-beer">Füge jetzt dein erstes hinzu!</Link>
        </p>
      ) : (
